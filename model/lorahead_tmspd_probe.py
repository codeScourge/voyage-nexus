...
class LoRAConv1x1(nn.Module):
    """Low-rank adapter for a 1x1 Conv2d (the trunk's pointwise feature-mixing layer,
    sep_point: fused_maps -> F2). Base weight frozen; learns W = W0 + (alpha/r)*B@A.
    Forward keeps the base conv's stride/padding/groups."""
    def __init__(self, base, r=4, alpha=8.0):
        super().__init__()
        w = base.weight.detach().clone()             # (O, I, 1, 1)
        self.W0 = w; self.W0.requires_grad_(False)
        self.b0 = base.bias.detach().clone() if base.bias is not None else None
        if self.b0 is not None:
            self.b0.requires_grad_(False)
        self.stride, self.padding, self.groups = base.stride, base.padding, base.groups
    ...
    def _delta(self):
        return (self.B @ self.A).view(self.W0.shape[0], self.W0.shape[1], 1, 1)

    def forward(self, x):
        w = self._w0(x.device) + self.s * self._delta()
        return F.conv2d(x, w, self.b0, self.stride, self.padding, groups=self.groups)


class DoRAConv1x1(LoRAConv1x1):
    """Weight-decomposed LoRA on the same 1x1 conv: per-output-channel magnitude m,
    LoRA on the direction."""
    def __init__(self, base, r=4, alpha=8.0):
        super().__init__(base, r, alpha)
        o, i = self.W0.shape[0], self.W0.shape[1]
        self.m = nn.Parameter(self.W0.view(o, i).norm(dim=1).detach().clone())

    def forward(self, x):
        o, i = self.W0.shape[0], self.W0.shape[1]
        v = self._w0(x.device).view(o, i) + self.s * (self.B @ self.A)
        w = (self.m.unsqueeze(1) * v / (v.norm(dim=1, keepdim=True) + 1e-6)).view(o, i, 1, 1)
        return F.conv2d(x, w, self.b0, self.stride, self.padding, groups=self.groups)


def _inject_trunk_lora(model, mode, r, alpha):
    """Replace the trunk's pointwise mixing conv (model.base.sep_point) with a LoRA/DoRA
    adapter. Returns the adapter's trainable params. sep_point is the only valid LoRA
    target: spatial/sep_depth are depthwise (no channel mixing) and temporal has in=1."""
    base_conv = model.base.sep_point
    new = (LoRAConv1x1(base_conv, r, alpha) if mode == "lora"
           else DoRAConv1x1(base_conv, r, alpha)).to(next(model.parameters()).device)
    model.base.sep_point = new
    return [p for p in new.parameters() if p.requires_grad]


def adapt_and_score(base, arm, e_null, Xe_t, Xm_t, y, cal, test, n_classes, args, tag):
    """Deep-copy the trained conditioned model, adapt ONE part of it on the k labeled
    calibration trials, score the held-out remainder. Held-out subject's embedding is
    fixed at the null token for every non-embed arm, so the arms differ only in WHAT is
    adapted:
        embed  -> the 16-d subject tag only      (trunk + head frozen)
        linear -> the readout head only          (trunk frozen, e=null)
        lora   -> LoRA on trunk sep_point only   (head frozen, e=null)
        dora   -> DoRA on trunk sep_point only   (head frozen, e=null)
        full   -> entire trunk + head            (e=null)
    Trunk stays in eval() during adaptation (frozen BN stats, dropout off) for few-shot
    stability; only the targeted weights move."""
    model = copy.deepcopy(base).to(DEVICE)
    for p in model.parameters():
        p.requires_grad_(False)

    if arm == "embed":
        e = e_null.clone().detach().to(DEVICE).requires_grad_(True)
        params = [e]; lr, steps = args.cal_lr, args.cal_steps
    else:
        e = e_null.clone().detach().to(DEVICE)            # fixed null token
        lr, steps = args.peft_lr, args.peft_steps
        if arm == "linear":
            for p in model.head.parameters():
                p.requires_grad_(True)
            params = list(model.head.parameters())
        elif arm in ("lora", "dora"):
            params = _inject_trunk_lora(model, arm, args.peft_r, args.peft_alpha)
        elif arm in ("lora+head", "dora+head"):
            # standard PEFT config: trunk adapter AND the readout head train together
            mode = arm.split("+")[0]                          # "lora" or "dora"
            adapter_params = _inject_trunk_lora(model, mode, args.peft_r, args.peft_alpha)
            for p in model.head.parameters():
                p.requires_grad_(True)
            params = adapter_params + list(model.head.parameters())
        elif arm == "full":
    ...
    model.eval()                                          # frozen BN/dropout
    crit = nn.CrossEntropyLoss()
    opt = torch.optim.Adam(params, lr=lr)
    Xe_c, Xm_c = Xe_t[cal].to(DEVICE), Xm_t[cal].to(DEVICE)
    yc = torch.from_numpy(y[cal]).long().to(DEVICE)
    for st in range(steps):
        opt.zero_grad()
        out = model.forward_with_embedding(Xe_c, Xm_c, e)
        loss = crit(out, yc); loss.backward(); opt.step()
        if args.log_every and ((st + 1) % max(1, steps // 4) == 0 or st == 0):
            print(f"        {tag}[{arm}] step {st+1:3d}/{steps} loss={loss.item():.4f}", flush=True)
    ...
    with torch.no_grad():
        ...
        for s0 in range(0, len(te), 256):
            j = te[s0:s0 + 256]
            o = model.forward_with_embedding(Xe_t[j].to(DEVICE), Xm_t[j].to(DEVICE), e)
            preds.append(o.argmax(1).cpu().numpy())
        yp = np.concatenate(preds)
    return compute_metrics(y[test], yp, n_classes, "_")["balanced_accuracy"]


def _cold_score(base, e_null, Xe_t, Xm_t, y, test, n_classes):
    """k=0 floor: predict the full held-out subject with the null token, no adaptation.
    Identical for every arm -> the shared cold floor."""
    base.eval()
    with torch.no_grad():
        preds = []
        te = torch.as_tensor(test)
        for s0 in range(0, len(te), 256):
            j = te[s0:s0 + 256]
            o = base.forward_with_embedding(Xe_t[j].to(DEVICE), Xm_t[j].to(DEVICE),
                                            e_null.to(DEVICE))
            preds.append(o.argmax(1).cpu().numpy())
        yp = np.concatenate(preds)
    return compute_metrics(y[test], yp, n_classes, "_")["balanced_accuracy"]
...
def run_kshot_embed(Xe, Xm, y, groups, keep, args):
    """LOSO. Per held-out subject: train the lookup-conditioned model on the others (ONE
    shared base), then for each k draw a calibration/test split ONCE and run every --arms
    calibration strategy on that identical split. k=0 is the shared cold floor (null
    token, no adaptation). Arms span the capacity ladder: embed (16-d tag) -> linear
    (head) -> lora/dora (trunk sep_point) -> full (whole trunk+head)."""
    n_classes = len(keep); T = Xe.shape[2]; chance = 1.0 / n_classes
    uniq = sorted(np.unique(groups).tolist())
    ks = [int(x) for x in args.kshots.split(",")]
    arms = [a.strip() for a in args.arms.split(",") if a.strip()]
    valid = {"embed", "linear", "lora", "dora", "lora+head", "dora+head", "full"}
    bad = set(arms) - valid
    if bad:
        raise ValueError(f"unknown arms {bad}; choose from {sorted(valid)}")
    Xe_t = torch.from_numpy(Xe).unsqueeze(1).float()
    Xm_t = torch.from_numpy(Xm).unsqueeze(1).float()
    res = {(arm, k): [] for arm in arms for k in ks}        # per-arm per-k bal_acc lists
    print(f"\n==== K-SHOT CALIBRATION LADDER (LOSO over {uniq}, words {[c+1 for c in keep]}, "
          f"chance={chance:.3f}) ====")
    print(f"  arms={arms}  cal_draws={args.cal_draws}")
    print(f"  embed: {args.cal_steps} steps @ lr {args.cal_lr} | peft arms: {args.peft_steps} "
          f"steps @ lr {args.peft_lr} (lora/dora r={args.peft_r}, alpha={args.peft_alpha}, "
          f"target=base.sep_point)")
    print("  k=0 = null token, no adaptation = shared cold floor (same for all arms).")
    for g in uniq:
        ...
        for k in ks:
            if k == 0:
                bal = _cold_score(lk, e_null, Xe_t, Xm_t, y, te_all, n_classes)
                for arm in arms:
                    res[(arm, 0)].append(bal)
                print(f"  {tag} k=0  cold floor bal_acc={bal:.4f}  (all arms, test_n={len(te_all)})",
                      flush=True)
                continue
            per_arm = {arm: [] for arm in arms}
            for d in range(args.cal_draws):
                rng = np.random.default_rng(args.seed * 1000 + d)
                cal, test = draw_cal(te_all, y, k, n_classes, rng)   # SAME split for all arms
                for arm in arms:
                    per_arm[arm].append(
                        adapt_and_score(lk, arm, e_null, Xe_t, Xm_t, y, cal, test,
                                        n_classes, args, tag + f"[k{k}d{d}]"))
            row = []
            for arm in arms:
                m = float(np.mean(per_arm[arm])); res[(arm, k)].append(m)
                row.append(f"{arm}={m:.4f}")
            print(f"  {tag} k={k:<2d}  " + "  ".join(row)
                  + f"   (draws={args.cal_draws}, test_n={len(test)})", flush=True)
        del lk
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

    print(f"\n==== K-SHOT LADDER SUMMARY (mean over {len(uniq)} held-out subjects) ====")
    print(f"chance={chance:.3f}   arms = capacity ladder (what each adapts)")
    cold0 = float(np.mean(res[(arms[0], 0)])) if 0 in ks else None
    head = f"{'k':>3}" + "".join(f"{a:>12}" for a in arms)
    if "embed" in arms:
        head += "".join(f"{a+'-emb':>14}" for a in arms if a != "embed")
    print(head)
    for k in ks:
        cells = [f"{k:>3}"]
        means = {arm: float(np.mean(res[(arm, k)])) for arm in arms}
        for arm in arms:
            cells.append(f"{means[arm]:>12.4f}")
        if "embed" in arms:
            for arm in arms:
                if arm != "embed":
                    cells.append(f"{means[arm]-means['embed']:>+14.4f}")
        print("".join(cells))
    if cold0 is not None:
        print(f"\ncold floor (k=0) = {cold0:.4f}. Per arm, gain over cold floor at max k:")
        kmax = ks[-1]
        for arm in arms:
            print(f"  {arm:>7}: {np.mean(res[(arm, kmax)]) - cold0:+.4f}")
    print("\nnote: trailing '*-emb' columns = arm minus embed at the same k (the 'other half'\n"
          "      a trunk/full adapter recovers ON TOP OF the 16-d tag). Test set shrinks by\n"
          "      k*W as k grows; compare the trend. Trunk stays in eval() during adaptation;\n"
          "      full fine-tune on tiny k will overfit -- that's expected and shows in the curve.")
    return res
...
def main():
    ...
    ap.add_argument("--arms", default="embed,linear,lora,dora,full",
                    help="calibration arms (subset of embed,linear,lora,dora,lora+head,"
                         "dora+head,full). embed=16-d tag; linear=head; lora/dora=trunk "
                         "sep_point (head frozen); lora+head/dora+head=trunk sep_point AND "
                         "head (standard PEFT config); full=whole trunk+head.")
    ap.add_argument("--cal-draws", type=int, default=3, help="random calibration draws to average (k>0)")
    ap.add_argument("--cal-steps", type=int, default=200, help="Adam steps to fit e_new (embed arm)")
    ap.add_argument("--cal-lr", type=float, default=1e-2, help="lr for the e_new fit (embed arm)")
    ap.add_argument("--peft-steps", type=int, default=100, help="Adam steps for linear/lora/dora/full arms")
    ap.add_argument("--peft-lr", type=float, default=1e-3, help="lr for linear/lora/dora/full arms")
    ap.add_argument("--peft-r", type=int, default=4, help="LoRA/DoRA rank on trunk sep_point")
    ap.add_argument("--peft-alpha", type=float, default=8.0, help="LoRA/DoRA alpha (scale = alpha/r)")
