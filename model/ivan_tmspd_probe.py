...
def main():
    ...
    ap.add_argument("--experiment", choices=["sweep", "unseen-embed", "transfer"], default="sweep",
                    help="sweep = baseline/EA/cond (default). unseen-embed = Q2 (populate an "
                         "unseen subject's embedding: mean/null/soft/amort + buffer sweep). "
                         "transfer = Q3 (single-source transfer matrix + Riemannian-similarity Mantel).")
    ...
    if args.experiment in ("unseen-embed", "transfer"):
        ...
        if len(np.unique(g)) < 2:
            ap.error("need >=2 subjects for unseen-embed/transfer")
