...
def run_kshot_embed(Xe, Xm, y, groups, keep, args):
    ...
    valid = {"embed", "linear", "lora", "dora", "full"}
    bad = set(arms) - valid
    if bad:
    ...
    head = f"{'k':>3}" + "".join(f"{a:>10}" for a in arms)
    if "embed" in arms:
        head += "".join(f"{a+'-emb':>11}" for a in arms if a != "embed")
    print(head)
    for k in ks:
        cells = [f"{k:>3}"]
        means = {arm: float(np.mean(res[(arm, k)])) for arm in arms}
        for arm in arms:
            cells.append(f"{means[arm]:>10.4f}")
        if "embed" in arms:
            for arm in arms:
                if arm != "embed":
                    cells.append(f"{means[arm]-means['embed']:>+11.4f}")
        print("".join(cells))
...
def main():
    ...
    ap.add_argument("--arms", default="embed,linear,lora,dora,full",
                    help="calibration arms to run (subset of embed,linear,lora,dora,full). "
                         "embed=16-d tag; linear=head; lora/dora=trunk sep_point; full=trunk+head.")
